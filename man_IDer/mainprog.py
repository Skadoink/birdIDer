...
def detect_fn(image):
    ...
    print("time for preprocess: " + str(time.time() - current_time)); current_time = time.time()

    detections = detection_model(image, training = False)
    print("time for detections: " + str(time.time() - current_time)); current_time = time.time()
...
def scan_image(image_np, x):
    ...
    detections, shapes = detect_fn(input_tensor)
